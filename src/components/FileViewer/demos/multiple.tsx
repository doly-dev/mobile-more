...
import { Button, ImageUploadItem } from 'antd-mobile';
...
function Demo() {
  const fileRef = React.useRef<ImageUploadItem>();
  const [open, setOpen] = React.useState(false);

  const preview = (type: keyof typeof data) => {
    fileRef.current = data[type];
    setOpen(true);
  };
  ...
  return (
    <div>
      ...
      <FileViewer file={fileRef.current} visible={open} onClose={() => setOpen(false)} />
    </div>
  );
}
